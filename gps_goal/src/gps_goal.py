...
class GpsGoal():
  ...
  def do_gps_goal(self, goal_lat, goal_long, z=0, yaw=0, roll=0, pitch=0):
    ...
    rospy.loginfo('the goal latitude data type: %s and value is %f', type(goal_lat), goal_lat)
    rospy.loginfo('the goal longitude data type: %s and value is %f', type(goal_long), goal_long)
    rospy.loginfo('the origin latitude data type: %s and value is %f', type(self.origin_lat), self.origin_lat)
    rospy.loginfo('the origin latitude data type: %s and value is %f', type(self.origin_long), self.origin_long)
  ...
  def gps_goal_fix_callback(self, data):
    rospy.loginfo(' Executing waypoints starting with, %f, %f' % (data.latitude, data.longitude))
    rospy.loginfo('the latitude data type: %s', type(data.latitude))
